...
def validate_client_id(value: str) -> str:
    ...
    if not re.fullmatch(r"[A-Za-z0-9._:-]{16,128}", client_id):
        raise ValueError("A valid device session id is required.")
    return client_id
...
class JobManager:
    ...
    def list(self, client_id: str) -> list[dict]:
        with self.lock:
            owned = (job for job in self.jobs.values() if job.get("client_id") == client_id)
            return [self._public(job, include_logs=False) for job in sorted(
                owned, key=lambda item: item.get("created_at", ""), reverse=True
            )]

    def get(self, job_id: str, client_id: str) -> dict | None:
        with self.lock:
    ...
    def create(self, payload: dict, client_id: str) -> dict:
        ...
        return self.get(job_id, client_id)
    ...
    def stop(self, job_id: str, client_id: str) -> dict | None:
        with self.lock:
            job = self.jobs.get(job_id)
            if not job or job.get("client_id") != client_id:
                return None
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def _require_client_id(self) -> str | None:
        try:
            return validate_client_id(self.headers.get("X-Client-ID", ""))
    ...
    def do_GET(self):
        ...
        if path == "/api/jobs" or path.startswith("/api/jobs/"):
            ...
            if not client_id:
                return
        if path == "/api/jobs":
            return self._json({"jobs": MANAGER.list(client_id)})
        match = re.fullmatch(r"/api/jobs/([^/]+)", path)
        if match:
            job = MANAGER.get(match.group(1), client_id)
    ...
    def do_POST(self):
        ...
        if path == "/api/jobs":
            try:
                job = MANAGER.create(self._read_json(), client_id)
        ...
        if match:
            job = MANAGER.stop(match.group(1), client_id)
            return self._json(job if job else {"error": "Job not found."}, HTTPStatus.OK if job else HTTPStatus.NOT_FOUND)
        return self._json({"error": "Endpoint not found."}, HTTPStatus.NOT_FOUND)
